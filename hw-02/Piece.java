...
public class Piece {
	...
	*/
	...
	public Piece computeNextRotation() {
		TPoint[] rotated_body = new TPoint[body.length];
		for(int i =0; i < body.length; i++){
			TPoint tmp = body[i];
			rotated_body[i] = new TPoint(height - 1 - tmp.y, tmp.x);
		}
		return new Piece(rotated_body);
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		return comparePointsArray(this.body, other.getBody());
	}

	private static boolean comparePointsArray(TPoint[] a, TPoint[] b){
		HashSet<String> stringBody = new HashSet<>();
		for(TPoint p : a)
			stringBody.add(p.toString());
		for(TPoint p : b)
			if(!stringBody.contains(p.toString())) return false;
		return true;
	}
...
//		Piece iter = root.computeNextRotation();
//		Piece savedNext = iter;
//		root.next = iter;
//		while(!iter.equals(root)){
//			iter.next = iter.computeNextRotation();
//			iter = iter.next;
//		}
//		iter.next = savedNext;
		Piece currPiece = root.computeNextRotation();
		root.next = currPiece;
		Piece next = currPiece.computeNextRotation();
